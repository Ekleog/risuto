    ...
    pub fn parse_search(db: &DbDump, pairs: Pairs<Rule>) -> Option<Query> {
        SEARCH_PARSER
            .map_primary(|p| match p.as_rule() {
                Rule::archived => Some(Query::Archived(match p.into_inner().next().map(|p| p.as_rule()) {
            ...
            .map_infix(|lhs, op, rhs| match op.as_rule() {
                Rule::and => lhs.and_then(|lhs| rhs.map(|rhs| Query::All(vec![lhs, rhs]))),
                Rule::or => lhs.and_then(|lhs| rhs.map(|rhs| Query::Any(vec![lhs, rhs]))),
...
impl Query {
    pub fn from_search(db: &DbDump, search: &str) -> Query {
        ...
        match search_parser::Parser::parse(search_parser::Rule::everything, search) {
            Ok(mut pairs) => {
                eprintln!("parser intermediate pairs: {:?}", pairs);
                // ignore the Pair generated by EOI
                let search_res = pairs.next().expect("Rule::everything result without search result");
...
mod tests {
    ...
    fn example_db() -> DbDump {
        ...
        tags.insert(TagId(Uuid::new_v4()), (Tag { owner: UserId::stub(), name: String::from("foo"), archived: false }, AuthInfo::all()));
        tags.insert(TagId(Uuid::new_v4()), (Tag { owner: UserId::stub(), name: String::from("bar"), archived: false }, AuthInfo::all()));
        tags.insert(TagId(Uuid::new_v4()), (Tag { owner: UserId::stub(), name: String::from("baz"), archived: false }, AuthInfo::all()));
        DbDump {
            owner: UserId::stub(),
            users: HashMap::new(),
            tags,
    ...
    fn primaries() {
        let db = example_db();
        assert_eq!(Query::from_search(&db, "archived:true"), Query::Archived(true));
    }

    #[test]
    fn infixes() {
        let db = example_db();
        assert_eq!(Query::from_search(&db, "archived:true archived:false"), Query::All(vec![Query::Archived(true), Query::Archived(false)]));
    }
}
